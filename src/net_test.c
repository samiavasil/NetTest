...
#include"test_cfg.h"
    ...
    void
print_macs(struct ether_header* eh)
{
    mac_addr_t mac_d = {0};
    mac_addr_t mac_s = {0};
...
do_sniff(connection_ctx_t*  con_ctx)
{
	...
	rval = recvfrom(con_ctx->fd->fd, buf, sizeof(buf), 0,
			(struct sockaddr*)&rcvaddr, &lens);
	if (rval < 0) {
    ...
    static void
init_master_ctx(master_ctx_t* mctx)
{
    ...
    static int
process_new_connection(master_ctx_t* mctx)
{
    int ret = 0;
    int    new_sd = -1;
    ...
    static int
default_svc(connection_ctx_t* ctx)
{
...
connect_client(connection_ctx_t* ctx)
{
    ...
    static int
process_clients(master_ctx_t* mctx)
{
...
fun()
{
	...
	if (FINISHED == test_descs[test_counter].hndl(
		&test_descs[test_counter].context)) {
...
run_master_service(char* eth)
{
    ...
    } while (end_program == FALSE); /* End of serving running.    */

    fprintf(stderr, "exit normal\n");
    /* Return old promiscuous mode*/
    if (!(old_if_mode & IFF_PROMISC)) {
	printf("Revert to previous promiscuous mode\n");
